...
public class Nif {
	...
	public void setTexto(String texto) throws ModeloException {
		...
		if (formatoValido(texto) && letraNIFValida(texto)) {
			...
			throw new ModeloException("Nif: formato o letra no válida.");
		}
	}
	 ...
	 * @param texto del NIF
	 * @return true si la letras es correcta.
	 */
	private boolean letraNIFValida(String texto) {
		int numeroNIF = Integer.parseInt(texto.substring(0,8));
		if (texto.charAt(8)== "TRWAGMYFPDXBTNJZSQVHLCKE".charAt(numeroNIF % 23)){
			return true;
		}
		return false;
	}
